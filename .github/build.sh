...
# Global Settings.
BASE_NAME='libmatrixmath';   # Base name for the project.
PROJECT_PATH=$(pwd);         # Root path of the project.

# Dependencies for tests and library (add as needed).
TEST_DEPENDENCIES='-lm -ljson -lstr';
LIBRARY_DEPENDENCIES='-lm -ljson -lstr';
...
if ! [ -f /usr/local/lib/libstr.so ] || ! [ -f /usr/local/lib/libjson.so ]; then
